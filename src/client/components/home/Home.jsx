...
export default function Home() {
  ...
  return <div className="product-home" data-testid="product-home">
    <section className="product-hero product-container">
    ...
    <section className="product-facts" aria-label={t('product')}><div className="product-container">{FACTS.map((Icon, index) => <div key={index}><Icon/><div><strong>{t(`facts.title${index}`)}</strong><span>{t(`facts.body${index}`)}</span></div></div>)}</div></section>
    <section className="product-section product-container" id="features"><div className="product-section-heading"><div><span className="product-eyebrow">{t('featuresEyebrow')}</span><h2>{t('featuresTitle')}</h2></div><p>{t('featuresBody')}</p></div><div className="product-features">{FEATURES.map((Icon, index) => <article className="product-feature" key={index}><div className="product-feature-top"><span className="product-feature-icon"><Icon/></span><span className="product-feature-number">0{index + 1}</span></div><h3>{t(`featureTitles.${index}`)}</h3><p>{t(`featureBodies.${index}`)}</p></article>)}</div><p className="product-availability"><TuneOutlined fontSize="small"/>{t('availability')}</p></section>
    <section className="product-roles"><div className="product-container product-roles-inner"><div><span className="product-eyebrow">{t('rolesEyebrow')}</span><h2>{t('rolesTitle')}</h2><p>{t('rolesBody')}</p><Link className="product-text-link" to="/login">{t('accessWorkspace')}<ArrowForward fontSize="small"/></Link></div><div className="product-role-list">{[0, 1, 2].map(index => <article key={index}><span>0{index + 1}</span><div><h3>{t(`roleTitles.${index}`)}</h3><p>{t(`roleBodies.${index}`)}</p></div><ArrowOutward aria-hidden="true"/></article>)}</div></div></section>
    <section className="product-section product-container product-faq" id="faq"><div><span className="product-eyebrow">{t('faqEyebrow')}</span><h2>{t('faqTitle')}</h2><p>{t('faqBody')}</p></div><div>{[0, 1, 2, 3, 4].map(index => <details key={index}><summary>{t(`questions.${index}`)}<span aria-hidden="true">+</span></summary><p>{t(`answers.${index}`)}</p></details>)}</div></section>
    <section className="product-container product-closing"><div><span className="product-eyebrow">{t('closingEyebrow')}</span><h2>{t('closingTitle')}</h2><p>{t('closingBody')}</p></div><a className="product-button" href={BRAND.salesHref || '#preview'}>{t(BRAND.salesHref ? 'requestDemo' : 'explore')}<ArrowForward fontSize="small"/></a></section>
  </div>;
}
