    ...
    <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 1100, mx: 'auto' }}>
      <Stack direction="row" spacing={1.5} alignItems="center" sx={{ mb: 3 }}>
        ...
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">Campus-wide attendance records</Typography>
        </Box>
      </Stack>
            ...
            <TableBody>
              {loading
                ? Array.from({ length: 6 }).map((_, i) => (
                    <TableRow key={i}>{Array.from({ length: 4 }).map((__, j) => <TableCell key={j}><Skeleton /></TableCell>)}</TableRow>
                  ))
                : records.length === 0
                  ? (
                    <TableRow>
